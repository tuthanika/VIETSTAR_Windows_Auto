...
function Invoke-DownloadRule {
    ...
    # Build correct remote base path per label (fix: use $env:driver/$env:boot7/$env:silent correctly)
    $basePathVar = Get-Item "env:$baseEnvVar"
    $basePathVal = $basePathVar.Value
    ...
    try {
        ...
        # Only parse when output looks like JSON (starts with '[')
        if ($jsonMain -match '^\s*\[') {
            try {
                $files = $jsonMain | ConvertFrom-Json
            } catch {
                Write-Warning "[WARN][$label] ConvertFrom-Json failed: $($_.Exception.Message)"
                $files = $null
            }
            if ($files) {
                ...
                if ($lastFile) {
                    ...
                    # Alist path (kept as-is; only fixing rclone remoteDir per request)
                ...
                Write-Host "[DEBUG][$label] No files parsed from JSON."
            }
        } else {
